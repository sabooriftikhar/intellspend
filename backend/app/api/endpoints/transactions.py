...
@router.get("/summary", response_model=schemas.transaction.TransactionSummary)
def read_transaction_summary(
    ...
    return crud.transaction.summarize_transactions(
        db,
...
def read_transactions(
    response: Response,           # ← non-default; must come before optional query params
    book_id: int | None = None,
    account_id: int | None = None,
    category_id: int | None = None,
    start_date: date | None = None,
    ...
    total = crud.transaction.count_transactions(
        db,
        user_id=current_user.id,
        book_id=book_id,
        account_id=account_id,
...
def update_transaction(
    ...
    db_transaction = crud.transaction.update_transaction(
        db, transaction_id=transaction_id, transaction=transaction, user_id=current_user.id
    )
    if db_transaction is None:
        raise HTTPException(status_code=404, detail="Transaction not found")
    return db_transaction
...
def delete_transaction(
    ...
    db_transaction = crud.transaction.delete_transaction(
